...
const generatePassword = () => {
  ...
  const UppercaseChars = includeUppercase.value ? "ABCDEFGHIJKLMNOPQRSTUVXYZ" : ""
  const numberChars = includeNumbers.value ? "0123456789" : ""
  const symbolChars = includeSymbols.value ? "1@#$%^&*()_+[]{}|;:,.<>?/~`" : ""
...
<template>
  <div class="password-generator-container">
    ...
    <div v-if="generatedPassword" class="generated-password">
      <strong style="font-weight: bold;">Your Password:</strong> {{ generatedPassword }}
    </div>
  </div>
</template>
